...
//shaders for mesh rendering and light gizmo
...
//uncomment to see candy wrapper artefacts on dwarf shoulder
...
void OnInit() {
	...
	//if dy>dz, the model is Yup
	bYup = (dy>dz);

	//get the combined bone transform
	UpdateCombinedMatrices();

	//resize bind pose, inverse bind pose and animatedXform vectors
	...
	shader.Use();
		...
		//pass values to uniforms at initialization
	...
	glBindVertexArray(vaoID);
		...
		GL_CHECK_ERRORS
		//enable vertex attribute array for blend weights 
		...
		GL_CHECK_ERRORS
		//enable vertex attribute array for blend indices 
...
void OnIdle() {
	...
	//pass the new animated transforms to the shader by updating the uniform
	shader.Use();
		glUniformMatrix4fv(shader("Bones"), animatedXform.size(), GL_FALSE, glm::value_ptr(animatedXform[0]));
	shader.UnUse();
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("EZMesh Skeletal Animation Viewer - OpenGL 3.3");
